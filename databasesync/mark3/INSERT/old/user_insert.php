...
$failmessage_user = "userfail";
$failmessage_connection = "connectionfail";
$successmessage = "success";


$passswordhash = password_hash($password, PASSWORD_DEFAULT);





$insert_query=("INSERT INTO user (nickname, firstname, lastname, birth, code, password) VALUES ('$nickname', '$firstname', '$lastname', $birth', '$code', '$passwordhash'");
...
if($check_execution_nickname === false) {
	...
	die(mysqli_error($con, $check_query));
	print $failmessage_connection;
	
}else{
	if(mysqli_num_rows($check_execution_nickname) == 0) {

	if(mysqli_num_rows($check_execution_email) == 0){

	mysqli_query($con, $insert_query);
	print $successmessage;

}else{ print $failmessage_email;
